...
<html>
  ...
  <form method="post" action="/Admin/Index/smoneysupport/">
   ...
   <div class="lists">
    <?php if(is_array($smoneysupport)): $i = 0; $__LIST__ = $smoneysupport;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$smoneysupport): $mod = ($i % 2 );++$i;?><div class="list-item">
     <div class="row bor-b no-gutter">
    <div class="col-80">
     <ul>
     <li>申请日期：<?php echo ($smoneysupport["date"]); ?></li>
     <li>姓名：<?php $uid=$smoneysupport['uid'];$m=M('user');$profile=$m->where("uid='$uid'")->find();echo $profile['name']; ?></li>
     <li>手机：<?php echo ($profile["mobile"]); ?></li>
     <li>金额：<?php echo ($smoneysupport["price"]); ?>万</li>
     </ul>
    </div>
    <div class="col-20">
      <ul class="actions">
       <?php if($smoneysupport['status']==1): ?><li class="bor-l"><a href="/Admin/Index/domoneysupport/id/<?php echo ($smoneysupport["id"]); ?>/price/<?php echo ($moneysupport["price"]); ?>/uid/<?php echo ($moneysupport["uid"]); ?>/status/2/"><img src="/Public/template/admin/images/icon_gou0.png" alt=""/></a></li>
       <li class="bor-l bor-t"><a href="/Admin/Index/deletemoneysupport/id/<?php echo ($smoneysupport["id"]); ?>/"><img src="/Public/template/admin/images/icon_x0.png" alt=""/></a></li>
       <?php elseif($smoneysupport['status']==2): ?>
