...
<html>
  ...
  <form method="post" action="/Admin/Index/dosearchuser/">
   ...
   <div class="lists">
    <?php if(is_array($userlist)): $i = 0; $__LIST__ = $userlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$userlist): $mod = ($i % 2 );++$i;?><div class="list-item">
     <div class="row bor-b no-gutter">
    <div class="col-100">
     <ul>
     <li>姓名：<?php echo ($userlist["name"]); ?>　性别：<?php if($userlist['sex']==1): ?>男<?php else: ?>女<?php endif; ?>　出生日期：<?php echo ($userlist["birth"]); ?><br>手机号：<?php echo ($userlist["mobile"]); ?>　推荐人：<?php echo ($userlist["rmobile"]); ?><br>入会日期：<?php echo ($userlist["joindate"]); ?>　积分：<?php echo ($userlist["score"]); ?>　身份：<?php $id=$userlist['type'];$m=M('user_type');$usertype=$m->where("id='$id'")->find();if($usertype){ echo $usertype['title']; }else{ echo "会员"; } ?><br>地址：<?php echo ($userlist["address"]); ?>
</li>
     </ul>
    </div>
